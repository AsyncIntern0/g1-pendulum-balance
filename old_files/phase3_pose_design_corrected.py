"""
phase3_pose_design_corrected.py
===============================
...
# Reuse the EXACT Phase-1 scenario API.  The project has used both
# generate_fall_dataset_final.py and generate_fall_dataset.py names, so
# support either filename without changing the run_trial() contract.
try:
...
def main():
    ...
    # Fail early with a useful message if the Phase-1 module does not expose
    # the exact interfaces Phase 3 is designed to reuse.
    ...
    if missing_p1:
        raise RuntimeError(
            "Phase-1 module is missing required interfaces: "
            + ", ".join(missing_p1)
            + ". Phase 3 must use the same Phase-1 scenario implementation."
        )
