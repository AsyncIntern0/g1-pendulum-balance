...
def contact_peak_forces(model, data, ground_id, body_ids):
    """Peak resultant contact force this step, per tracked body, counting
    ONLY contacts against the ground geom (environmental impact) -- contacts
    between the robot's own links are ignored so folded/self-contact doesn't
    get mistaken for an impact."""
...
def run_protected_trial(model, scenario, magnitude, direction_deg, timing_phase_s,
    ...
    restore_model_state(model, snapshot)
    trigger_t_rel = max(0.0, (t_impact if t_impact is not None else 0.0) - trigger_lead_s)
    ...
    while step < max_steps:
        ...
        forces = contact_peak_forces(model, d, ground_id, impact_ids)
...
def score_pose(model, scenario, pose_ctrl, conditions, impact_ids, lead_time_s, snapshot):
    ...
    return total / len(conditions), diagnostics
...
def optimize_pose_for_scenario(model, scenario, lead_time_s, popsize, maxiter, snapshot, seed=0):
    ...
    best_score, best_diag = None, None
    while not es.stop():
        candidates = es.ask()
        fitnesses = []
        for x in candidates:
    ...
    return {
        ...
        "pose": best_x.tolist(), "score": float(best_score),
...
def main():
    ...
    ap.add_argument(
    "--model",
    default=r"C:\Users\Asyncronix\Downloads\Asyncronix_Intern\g1-pendulum-balance\unitree_g1\g1_pendulum.xml"
)
    ...
    print("EXISTS:", os.path.exists(args.model))
    ...
    if len(scenarios) > 1:
        ...
        cross_matrix = cross_evaluate(model, scenarios, raw_results, args.lead_time, snapshot)
        groups, representative = merge_library(scenarios, raw_results, cross_matrix)
        print(f"Merged {len(scenarios)} scenario-specific poses into "
    ...
    for pose_id, covered in groups.items():
        covered_scenarios = [scenarios[k] for k in covered]
        directions = sorted({
            _direction_label(s) for s in covered_scenarios
        })
        library.append({
            "pose_id": pose_id,
            "pose": raw_results[pose_id]["pose"],
            "covers_scenario_ids": [scenarios[k][0] for k in covered],
            "covers_scenario_fns": [scenarios[k][2] for k in covered],
            "fall_directions": directions,
            "own_score": raw_results[pose_id]["score"],
            "diagnostics": raw_results[pose_id]["diagnostics"],
        })
