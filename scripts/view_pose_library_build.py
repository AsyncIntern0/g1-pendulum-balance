...
def scenario_group(scenario):
    """Same scenario-to-group mapping used by build_pose_library.py."""
    sid = int(scenario[0])
    mapping = {
        1: "forward", 2: "backward", 3: "left", 4: "right",
        5: "unknown", 6: "unknown",
        7: "floor_tilt", 8: "floor_tilt",
        9: "sudden_load",
        10: "unknown", 11: "unknown", 12: "unknown", 13: "unknown",
        14: "forward", 15: "sudden_load",
    }
    return mapping.get(sid, "unknown")


def choose_bin(velocity):
    """
    build_pose_library.py stores low/mid/high bins, not velocity ranges.
    For visualization only, use simple thresholds. If the JSON later stores
    actual velocity bounds, replace this with those bounds.
    """
    v = float(velocity)
    if v < 1.0:
        return "low"
...
def select_build_library_pose(library, scenario, velocity):
    """
    Read the actual build_pose_library.py JSON format.

    Example key:
        forward/mid

    If the entry failed its gate (or is kind=stand), return None as the
    protected pose so the viewer can show that the library chose stand.
    """
    group = scenario_group(scenario)
    velocity_bin = choose_bin(velocity)
    key = f"{group}/{velocity_bin}"

    if key not in library:
        raise KeyError(
            f"No library entry '{key}'. Available keys: "
            + ", ".join(library.keys())
        )
    ...
    accepted = (
        entry.get("kind") == "pose"
        and bool(entry.get("gate_passed", False))
    )

    if not accepted:
        return None, entry, key

    if "pose_ctrl" not in entry:
        raise KeyError(f"Library entry '{key}' has no pose_ctrl.")

    pose = np.asarray(entry["pose_ctrl"], dtype=float)

    if pose.ndim != 1:
        raise ValueError(
            f"pose_ctrl for '{key}' must be 1-D; got {pose.shape}."
        )

    return pose, entry, key


def get_pose_id(entry, library_key):
    """The build library has no pose_id; use its group/bin key."""
    return library_key


def print_force_summary(label, peak):
    """Print peak contact forces for the completed viewer run."""
    print(f"\\n[{label} PEAK FORCES]")
...
def main():
    ...
    ap.add_argument(
        "--library",
        default=r"C:\Users\Asyncronix\Downloads\Asyncronix_Intern\g1-pendulum-balance\pose_library.json"
                ,
    )
    ...
    print("\n" + "=" * 68)
    print("BUILD_POSE_LIBRARY VISUAL COMPARISON")
    print("=" * 68)
    ...
    natural_impact, trigger_time, measured_velocity = (
        natural_impact_and_trigger(
            p1, model, scenario, args.magnitude,
            nominal_direction, args.timing,
        )
    )
    ...
    pose, meta, library_key = select_build_library_pose(
        library, scenario, lookup_velocity
    )

    pose_id = get_pose_id(meta, library_key)

    # If the builder rejected the candidate, the library explicitly says
    # "stand". Show the protected run using the standing command.
    if pose is None:
        pose = model.key_ctrl[0].copy()
        protected_is_pose = False
    else:
        protected_is_pose = True

    if pose.size != model.nu:
        raise ValueError(
            f"Selected command has {pose.size} values, "
            f"but XML has model.nu={model.nu} actuators."
        )
    ...
    print(f"Selected kind       : {meta.get('kind', '?')}")
    print(f"Gate passed         : {meta.get('gate_passed', False)}")
    print(
        "Protected command   : "
        + ("SELECTED POSE" if protected_is_pose else "STAND (candidate rejected)")
    )
