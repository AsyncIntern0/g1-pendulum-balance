...
# NO disturbance at all this time -- isolate the pose by itself
found = False
for step in range(int(1.0 / dt)):
    d.ctrl[:] = pose
    mujoco.mj_step(model, d)   # <-- no disturb_fn call here
    ...
    if new_bad:
        ...
        break
if not found:
    print("no new contact in 1.0s from the pose alone, no fault applied")
