"""
THIS INCLUDES THE CHANGES OF HAVING THE VELOCITY AXIS.
...
def optimize_pose_for_bin(model, scenario, magnitude, lead_time_s, popsize, maxiter,
    ...
    probe_score, probe_diag, probe_recovery = score_pose(model, scenario, x0, conditions, impact_ids,
    ...
    # Track (score, diag, recovery_rate) for the best candidate. Selection is
    ...
    best_score, best_diag, best_x, best_recovery = probe_score, probe_diag, x0, probe_recovery
    while not es.stop():
        candidates = es.ask()
        if pool is not None:
        ...
        for x, (s, diag, recovery_rate) in zip(candidates, results):
            ...
            if s == s and s < best_score:
                best_score, best_diag, best_x, best_recovery = s, diag, np.array(x), recovery_rate
...
def main():
    ...
    ap.add_argument("--out", default="phase3_out_v2",
    ...
    out_path = os.path.join(args.out, "pose_library_v2.json")
